...
All notable changes to the **Markdown Context Injector** extension will be documented in this file.
...
- **Configurable exclude list:**
  - `markdownContextInjector.useDefaultExcludes` (`boolean`, default `true`) — toggle for the built-in defaults (`node_modules`, `.git`, `build`, `out`, `dist`, `.next`, `.cache`, `.turbo`, `coverage`, `.fvm`, `.dart_tool`, `android`, `ios`, `.run`).
  - `markdownContextInjector.excludePatterns` (`string[]`, default `[]`) — additional glob patterns, merged with the defaults.
...
- **Dedicated `Markdown Context Injector` Output channel** for activation logs, index rebuild timings, and configuration-change diagnostics.
